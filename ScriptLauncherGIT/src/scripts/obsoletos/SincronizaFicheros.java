...
public class SincronizaFicheros extends RFCBasedScript {
	...
	public void exec() {
		...
		String selectedFilesPath = getRFCDirectory().concat(Constants.SELECTED_FILES_NAME);
	...
	public void sincroniza(Connection conn, boolean compilarSiNecesario, List<String> files) {
		...
		updatePrivateRepository(sincronizaFicheros(conn, environment, files));

		try {
			String lastSyncFilePath = getRFCDirectory().concat("ultimasinc.txt");
			//BufferedWriter out = new BufferedWriter(new FileWriter("ultimasinc.txt"));
		...
		if (procesadoFicherosJavaCustomUpstream && compilarSiNecesario) {
			...
			//SessionUtil.execRemoteCommandShowErr(conn.getSession(), PropertiesManager.getProperty(environment, "compileUpstreamCommand"));
		...
		if (procesadoFicherosJavaCoreUpstream && compilarSiNecesario) {
			...
			//SessionUtil.execRemoteCommandShowErr(conn.getSession(), PropertiesManager.getProperty(environment, "compileUpstreamCoreCommand"));
		...
		if (procesadoFicherosJavaCustomBuyer && compilarSiNecesario) {
			Display.log("Se han modificado ficheros java customizados-> compilado automatico ...");
			//SessionUtil.execRemoteCommandShowErr(conn.getSession(), PropertiesManager.getProperty(environment, "compileBuyerCustomCommand"));
		...
		if (procesadoFicherosJavaCoreBuyer && compilarSiNecesario) {
			Display.log("Se han modificado ficheros java core-> compilado automatico ...");
			// SessionUtil.execRemoteCommandShowErr(session, PropertiesManager.getProperty(environment,"compileUpstreamCommand"));
	...
	private void inicializacion(Connection conn) {
		...
		try {
			...
			//BufferedReader in = new BufferedReader(new FileReader("ultimasinc.txt"));
	...
	private String getRelativePathFromServer(String path, boolean isUpstream) {
		...
		if(isUpstream)
		{
			serverPath = PropertiesManager.getProperty("Local", "UpstreamPath");
		}
		else
		{
			serverPath = PropertiesManager.getProperty("Local", "BuyerPath");
		}
		
		String newPath = path.replace(serverPath, "");
		*/
	...
	private List<String> sincronizaFicheros(Connection conn, String environment, List<String> lista) {
		...
		while (it.hasNext()) {
			...
			if (SFTPUtil.getRemoteFile(conn.getSftpChannel(), remotePath, remoteFileOnLocal)) {
				...
				// El fichero existe en el sistema remoto y se ha descargado en remoteFileOnLocal
				if (!LocalUtil.areIdentical(file.getAbsolutePath(), remoteFileOnLocal)) {
					...
					try {
						// Crea copia del fichero para poder comprobar luego si se modificó el fichero durante el merge
						remoteFileCopyPath = remoteFileOnLocal + "bak";
						LocalUtil.copyfile(remoteFileOnLocal, remoteFileCopyPath);
						// Merge
						String fullCommand = compareCommand + " \"" + file.getAbsolutePath() + "\" \"" + remoteFileOnLocal + "\"";
				...
				// Crea los directorios por si no existen mediante la ejecución de un mkdir -p remoto
	...
	public void updatePrivateRepository(List<String> lista) {
		String impDirPath = LocalUtil.getImplementationDirectoryPath(getPackageDir(), getRFCName());
		Iterator<String> it = lista.iterator();
		while (it.hasNext()) {
			String path = it.next();
			boolean isUpstream = isUpstream(path);
			String localPath = LocalUtil.getLocalPath(getRelativePathFromServer(path, isUpstream), isUpstream);
			LocalUtil.copyfile(localPath, impDirPath + path);
		}
	}

	@Override
	public String getName() {
		return "Sincronizacion de ficheros";
	}
